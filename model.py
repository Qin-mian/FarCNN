...
class SpatialAttentionModule(nn.Module):
    def __init__(self, num_filters):
        ...
        self.attention_weights = nn.Conv2d(num_filters, num_filters, kernel_size=1, stride=1, padding=0)
        self.sigmoid = nn.Sigmoid()

    def forward(self, input_layer):
        attention_weights = self.attention_weights(input_layer)
        attention_weights = self.sigmoid(attention_weights)
        output = input_layer * attention_weights
        return output
...
class CNNModel(nn.Module):
    def __init__(self, image_shape):
        ...
        self.seed = 0  # Assuming SEED is defined somewhere, here it's hardcoded to 0
        ...
        self.sam = SpatialAttentionModule(128)
